...
export class CustomerTierController {
  ...
  @Get()
  @RequirePermissions({ module: PermissionModule.CUSTOMER_TIERS, permission: 'read' })
  ...
  @Post()
  @RequirePermissions({ module: PermissionModule.CUSTOMER_TIERS, permission: 'create' })
  ...
  @Patch(':code')
  @RequirePermissions({ module: PermissionModule.CUSTOMER_TIERS, permission: 'update' })
  @ApiOkResponse({ description: 'Customer tier updated successfully' })
  async updateTier(@Param('code') code: string, @Body() payload: UpdateCustomerTierDto) {
  ...
  @Delete(':code')
  @RequirePermissions({ module: PermissionModule.CUSTOMER_TIERS, permission: 'delete' })
  ...
  async listRules() {
    ...
    return ResponseUtil.success(data, 'Customer tier rules retrieved successfully');
  }
  ...
  async createRule(@Body() payload: CreateCustomerTierRuleDto) {
    ...
    return ResponseUtil.success(data, 'Customer tier rule created successfully');
  }

  @Patch('rules/:id')
  @RequirePermissions({ module: PermissionModule.CUSTOMER_TIERS, permission: 'update' })
  @ApiOkResponse({ description: 'Customer tier rule updated successfully' })
  async updateRule(@Param('id') id: string, @Body() payload: UpdateCustomerTierRuleDto) {
    const data = await this.customerTierService.updateRule(id, payload);
    return ResponseUtil.success(data, 'Customer tier rule updated successfully');
  }
  ...
  async deleteRule(@Param('id') id: string) {
    ...
    return ResponseUtil.success(null, 'Customer tier rule deleted successfully');
  }

  @Post('customers/:userId/override')
  @RequirePermissions({ module: PermissionModule.CUSTOMER_TIERS, permission: 'update' })
  @ApiOkResponse({ description: 'Customer tier override updated successfully' })
  async setOverride(@Param('userId') userId: string, @Body() payload: SetCustomerTierOverrideDto) {
    await this.customerTierService.setCustomerTierOverride(userId, payload.tierCode);
    return ResponseUtil.success(null, 'Customer tier override updated successfully');
  }
}
