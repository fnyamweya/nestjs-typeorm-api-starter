...
describe('PriceService', () => {
  ...
  it('resolves price for SKU using provided price list', async () => {
    ...
    const resolved = await service.resolveSkuPrice({ productSkuId: 'pv1', priceListId: '1', quantity: 1 });
  ...
  it('selects price list by currency precedence (priority)', async () => {
    ...
    const resolved = await service.resolveSkuPrice({ productSkuId: 'pv1', currencyCode: 'KES', quantity: 1 });
  ...
  it('caches resolved price', async () => {
    ...
    const r1 = await service.resolveSkuPrice({ productSkuId: 'pv-cache', currencyCode: 'KES', quantity: 1 });
    const r2 = await service.resolveSkuPrice({ productSkuId: 'pv-cache', currencyCode: 'KES', quantity: 1 });
  ...
  it('prefers context-matched prices over generic ones', async () => {
    ...
    priceRowRepo.find.mockResolvedValueOnce([
      ...
      { id: 'r5', unitAmount: '10000', minQuantity: 1, selectorJson: {} } as unknown as PriceRow,
    ]);
