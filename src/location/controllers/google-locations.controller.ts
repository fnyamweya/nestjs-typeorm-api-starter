...
export class GoogleLocationsController {
  ...
  @ApiQuery({ name: 'countryCode', required: false, type: String, description: 'ISO2 country code, e.g. KE' })
  @ApiQuery({ name: 'language', required: false, type: String, description: 'BCP-47 language code, e.g. en' })
  @ApiQuery({ name: 'sessionToken', required: false, type: String, description: 'Google Places session token' })
  ...
  @ApiOkResponse({ description: 'Google Place details mapped to address fields' })
  async details(
    @Query('placeId') placeId: string,
    @Query('language') language?: string,
    @Query('sessionToken') sessionToken?: string,
  ) {
    const details = await this.googlePlaces.placeDetails({ placeId, language, sessionToken });
  ...
  @Get('match')
  @ApiQuery({ name: 'placeId', required: true, type: String })
  @ApiQuery({ name: 'countryCode', required: false, type: String, description: 'ISO2 country code override, e.g. KE' })
  @ApiQuery({ name: 'language', required: false, type: String })
  @ApiQuery({ name: 'sessionToken', required: false, type: String })
  @ApiOkResponse({ description: 'Best-effort internal Location match preview (no persistence)' })
  async match(
    @Query('placeId') placeId: string,
    @Query('countryCode') countryCode?: string,
    @Query('language') language?: string,
    ...
    if (requestedCountry && googleCountry && requestedCountry !== googleCountry) {
    ...
    let match: { locationId: string; matchedType: string; matchedName: string } | null = null;
